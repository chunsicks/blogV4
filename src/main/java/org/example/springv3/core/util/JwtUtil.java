...
public class JwtUtil {
    ...
    //위조함
    public static String createX(User user){
        String accessToken = JWT.create()
                ...
                .sign(Algorithm.HMAC512("meta"));//지금은 RSA필요 없음 내가 암호하고 내가 확인하니까
        //시크릿이 중요하다 무조건!!
        return accessToken;
    }
    ...
    public static String createExp(User user){
        String accessToken = JWT.create()
                .withSubject("바보")//별로 안중요함 보라색의 첫번째 부분
                .withExpiresAt(new Date(System.currentTimeMillis() + 1000))//시간 중요함!!   토큰을 탈취했는데 상태 저장을 안해둠(얼굴이라도 저장) 할 수 있는 거는 싸인 맞네 (유효기간이 없으면 영원히) 그래서 짧게 잡음 10분 20분
                .withClaim("id", user.getId())  //payload자리
                .withClaim("username", user.getUsername())  //이거 없었는데 이거 넣으면 또 달라질거임
